...
def score_paper(paper: dict, weights: dict, feedback: dict, boosted_topics: list[str]) -> float:
    """
    Compute composite score after LLM analysis.
    Primary signal: lens from plain-text analysis + pre_score.
    ...
    score = paper.get("_pre_score", 0.0)

    # ── Tier 0: Boosted topics ──
    if any(topic in text for topic in boosted_topics):
        score += 5.0

    # ── Tier 1: Time-decayed feedback ──
    ...
    # ── Tier 2: Lens boost from plain-text LLM analysis ──
    lens = paper.get("lens", "GENERAL").upper()
    lens_boosts = {
        "CONTRADICTS_CONSENSUS": weights.get("contradicts_consensus", 3.0),
        "NEW_FRONTIER":          weights.get("new_frontier", 2.5),
        "CROSS_DISCIPLINARY":    weights.get("cross_disciplinary", 2.0),
        "GENERAL":               0.0,
    }
    score += lens_boosts.get(lens, 0.0)

    # ── Tier 3: Influential citations (log-scaled) ──
    infl = paper.get("influential_citations", 0) or 0
    if infl > 0:
        score += min(math.log1p(infl) * weights.get("influential_citations", 0.5), 2.0)

    # ── Tier 4: Full text available ──
...
def select_papers(papers: list[dict], config: dict, state_dir: str = "state") -> list[dict]:
    """
    Score all analyzed papers, apply diversity constraints, return top N.
    No min_score gate — papers are already pre-filtered to top N before analysis.
    """
...
def group_by_lens(papers: list[dict]) -> dict[str, list[dict]]:
    """Group selected papers by lens (plain string set by analyzer)."""
    ...
    for paper in papers:
        key = lens_map.get(paper.get("lens", "GENERAL").upper(), "general")
